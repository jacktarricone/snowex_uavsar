...
lidar_inc_deg_raw <-rast("/Volumes/JT/projects/uavsar/jemez/new_inc/lidar_inc_deg.tif")
plot(lidar_inc_deg_raw)
lidar_inc_deg_raw
...
lidar_inc <-resample(lidar_inc_deg_raw, amp1)

amp1 <-rast(insar_files[[1]])
values(amp1)[values(amp1) == 0] = NA
amp1 <-crop(amp1, lidar_inc)
amp1 <-mask(amp1, lidar_inc)
plot(amp1)

amp2 <-rast(insar_files[[2]])
values(amp2)[values(amp2) == 0] = NA
amp2 <-crop(amp2, lidar_inc)
amp2 <-mask(amp2, lidar_inc)
plot(amp2)

cor <-rast(insar_files[[3]])
values(cor)[values(cor) == 0] = NA
cor <-crop(cor, lidar_inc)
cor <-mask(cor, lidar_inc)
plot(cor)

dem <-rast(insar_files[[4]])
values(dem)[values(dem) == -10000] = NA
ext(dem) <-ext(cor)
dem <-mask(dem, lidar_inc)
plot(dem)

# lidar dem
lidar_dem <-rast("/Volumes/JT/projects/uavsar/jemez/lidar/lidar_uavsar_dem_resamp.tif")
lidar_dem <-resample(lidar_dem, lidar_inc) # not good
lidar_dem <-mask(lidar_dem, lidar_inc)
plot(lidar_dem)
