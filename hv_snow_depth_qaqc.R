...
del_thresh = 1.5 # 5mm
depth_data$delta_depth <- c(0, diff(depth_data$hv_snow_depth_cm))
...
depth_data$depth_lvl_2 <- ifelse(depth_data$delta_depth > del_thresh |
                             ...
                             NA,
                             depth_data$hv_snow_depth_cm)

# test plot of first filter of depth data
ggplot(depth_data) +
  geom_point(aes(x = date_time, y = hv_snow_depth_cm), size = .2)
...
depth_data$depth_lvl_3 <- ifelse(depth_data$delta_sd_interp > abs(del_thresh) |
...
ggplot(depth_data) +
  # geom_line(aes(x = date_time, y = hv_snow_depth_cm))+
...
write.csv(depth_data, "/Users/jacktarricone/ch1_jemez_data/climate_station_data/hv/hv_snow_depth_qaqc_v2.csv" )
