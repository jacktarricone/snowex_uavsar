...
lidar_deg <-crop(lidar_inc_v1, crop_ext)

### rad
# import i_angle raster and resample to unw grid bc of slight extent difference
lidar_inc_raw_rad <-rast("lidar_inc_rad.tif")
lidar_inc_v1_rad <-resample(lidar_inc_raw_rad, unw_raw)

# set crop extent and crop for better visualization
crop_ext <-ext(-106.57, -106.38, 35.81, 35.96) 
lidar_rad <-crop(lidar_inc_v1_rad, crop_ext)
#writeRaster(lidar_rad, "jemez_rad.tif")

# mask and crop unwrapped phase data down to extent on new inc angle raster
unw_crop <-crop(unw_raw, crop_ext)
unw <-mask(unw_crop, lidar_deg)
...
fsca_crop <-crop(fsca_raw, ext(lidar_deg))
fsca <-mask(fsca_crop, lidar_deg)
plot(fsca)
...
test_deg <-cos(lidar_deg)
plot(test_deg)

test_rad <-cos(lidar_rad)
plot(test_rad)


test <-(cos(lidar_rad) - sqrt(mean_k_feb12_20 - sin((lidar_rad)^2)))
plot(test)

test2 <-(cos(lidar_deg) - sqrt(mean_k_feb12_20 - sin((lidar_deg)^2)))
plot(test2)


# create the insar constant raster
# because of the variation of .inc angle, this value you will change on pixelwise basis
insar_constant_deg <-insar_constant(inc = lidar_deg, wL = uavsar_wL, k = mean_k_feb12_20)
hist(insar_constant_deg, breaks = 100)
plot(insar_constant_deg)

#do swe change calc with masked unwrapped phase data
dswe_deg <-insar_constant_deg*unw_snow_mask
plot(dswe_deg)
hist(dswe_deg, breaks = 100)
# writeRaster(dswe_raw,"./final_swe_change/raw_dswe_feb12_19.tif")

# create the insar constant raster
# because of the variation of .inc angle, this value you will change on pixelwise basis
insar_constant_rad <-insar_constant(inc = lidar_rad, wL = uavsar_wL, k = mean_k_feb12_20)
hist(insar_constant_rad, breaks = 100)
plot(insar_constant_rad)

#do swe change calc with masked unwrapped phase data
dswe_rad <-insar_constant_rad*unw_snow_mask
plot(dswe_rad)
hist(dswe_rad, breaks = 100)
# writeRaster(dswe_raw,"./final_swe_change/raw_dswe_feb12_19.tif")
...
# save
writeRaster(dswe_abs,"./final_swe_change/dswe_feb12-19_rad.tif")
