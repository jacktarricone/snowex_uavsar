...
# no snow unw
no_snow_unw <-mask(unw_masked, clipped_nsm, maskvalue = NA)
plot(no_snow_unw)

# no snow plv
no_snow_plv <-mask(plv_resamp, clipped_nsm, maskvalue = NA)
plot(no_snow_plv)

### convert no snow plv and unw rasters to dataframes, rename data columns
# unw
unw_df <-as.data.frame(no_snow_unw, xy=TRUE, cells=TRUE, na.rm=TRUE)
